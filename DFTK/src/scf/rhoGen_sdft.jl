...
function rhoGenChebP(ham, model, εF::Float64, M::Int64)
    ...
    X = []  # The type of X should be the Vector{Matrix{ComplexF64}} 
    for k = 1 :  length(ham.basis.kpoints)
        push!(X, Matrix{ComplexF64}) 
        npw = size(ham.blocks[k], 1)
        X[k] = zeros(ComplexF64, npw, 1)
    ...
    compute_density_sdft(ham.basis, X)
end 
...
function rhoGenStoc(ham, model, εF::Float64, M::Int64, Ns::Int64) 
    β = 1 / model.temperature
    X = []  # X should be the Vector{Matrix{ComplexF64}} 
    for k = 1 : length(ham.basis.kpoints)
        push!(X, Matrix{ComplexF64}) 
        npw = size(ham.blocks[k], 1)
        X[k] = zeros(ComplexF64, npw, 1)
...
# TODO: consistent with DFTK
# in DFTK
#occupation(S::FermiDirac, x) = 1 / (1 + exp(x))

"""Compute the density by a Vector

The difference with (compute_density) in DFTK is that all occupation are "1" here. Since we don't need to act again test function f on ψ (obtaind by Chebyshev method or Stochastic).
"""

@views @timing function compute_density_sdft(basis, ψ)
    ...
    @sync for (ichunk, chunk) in enumerate(Iterators.partition(ik_n, chunk_length))
        Threads.@spawn for (ik, n) in chunk  # spawn a task per chunk
            ...
            ρ_loc[:, :, :, kpt.spin] .+= basis.kweights[ik] .* abs2.(ψnk_real)
        end
    end
    ...
    #DFTK._check_positive(ρ)
    #n_elec_check = DFTK.weighted_ksum(basis, sum.(occupation))
    #DFTK._check_total_charge(basis.dvol, ρ, n_elec_check)
