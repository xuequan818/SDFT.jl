...
function run_mlmcpd_cost(L::Int, ϵs::Vector{Float64}; save_file=false, kws...)        
    var, Ql, ψ, basis, Cheb, ρ = run_mlmcpd_var(L; kws...)
    _run_mlmcpd_cost(ϵs, var, Ql, ψ, basis, Cheb, ρ, save_file)
end

function run_mlmcec_cost(L::Int, ϵs::Vector{Float64}; save_file=false, kws...)        
    var, Ql, ψ, basis, Cheb, ρ = run_mlmcec_var(L; kws...)
    _run_mlmcec_cost(ϵs, var, Ql, ψ, basis, Cheb, ρ, save_file)
end

for fun in [:run_mlmcpd_cost, :run_mlmcec_cost]
    _fun = Symbol("_", fun)
    @eval function $fun(data::Dict, ϵs::Vector{Float64}; save_file=false, kws...)
        ...
        $_fun(ϵs, var, Ql, ψ, basis, Cheb, ρ, save_file)
    end
end

function _run_mlmcpd_cost(ϵs::Vector{Float64}, var, Ql, ψ, basis, Cheb, ρ, save_file) 
    err_pd = Float64[]
    err_mc = Float64[]
    cost = []
    for (i, ϵ) in enumerate(ϵs)
        ...
        @time ρpd = compute_stoc_density(basis, 0, PDML; Cheb, ρ, ψin=ψ);
        push!(err_pd, norm(ρpd - ρ) * sqrt(basis.dvol))

        ns_mc = ceil(Int, sum(Ql .* pd_nsl) / Ql[end])
        @time ρmc = compute_stoc_density(basis, 0, MC(ns_mc); Cheb, ρ);
...
function _run_mlmcec_cost(ϵs::Vector{Float64}, var, Ql, ψ, basis, Cheb, ρ, save_file) 
    ...
    for (i, ϵ) in enumerate(ϵs)
        ...
        @time ρec = compute_stoc_density(basis, 0, ECML; Cheb, ρ, ψin=ψ);
