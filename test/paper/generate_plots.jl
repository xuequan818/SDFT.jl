...
begin
    DATA_DIR = "test/paper/data"
    IMAGE_DIR = "test/paper/images"
...
mlmc_pd_cost = let
    ...
    for (i, ic) in enumerate(CASES)
        ...
        ax.plot(xxs, fx.(xxs), label=L"\text{slope}=1"; linefit...)
...
mlmc_vs_dft = let
    for (i, ic) in enumerate(CASES_3D)
        ...
        fig.tight_layout()
        filename = joinpath(IMAGE_DIR, "wall_time_$i.pdf")
        fig.savefig(filename, bbox_inches="tight")
        println("Saved plot: $filename")
    end
end
...
mlmc_error = let
    for (i, ic) in enumerate(CASES_3D)
        ...
        fig.tight_layout()
        filename = joinpath(IMAGE_DIR, "mlmc_error_$i.pdf")
        fig.savefig(filename, bbox_inches="tight")
        println("Saved plot: $filename")
    end
end
...
mlmc_vs_mc = let
    for (i, ic) in enumerate(CASES_3D)
        data_mc = load(joinpath.(DATA_DIR, "mc_time_$(ic).jld2"))
        data_ec = load(joinpath.(DATA_DIR, "mlmcec_time_$(ic).jld2"))
        data_pd = load(joinpath.(DATA_DIR, "mlmcpd_time_$(ic).jld2"))
        ...
        ax.plot(xs, mc_time ./ ec_time, linestyle=":",
            linewidth=1,
            marker="o",
            markersize=8,
            label="energy cutoffs")
        ...
        fig.tight_layout()
        filename = joinpath(IMAGE_DIR, "speed_up_$i.pdf")
        fig.savefig(filename, bbox_inches="tight")
        println("Saved plot: $filename")
    end
end
